...
if [ $TESTNM != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		FILE1=$filename
		[ $? == 1 ] && exit 0;
		if [ "$FILE1" != "" -a "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" -a "$FILE1" != "/usr/lib/libsqlite3.dylib" -a "$FILE1" != "/usr/lib/php/extensions/no-debug-non-zts-20131226/opcache.a" -a "$FILE1" != "/usr/lib/php/extensions/no-debug-non-zts-20131226/opcache.so" -a "$FILE1" != "/usr/lib/php/extensions/no-debug-non-zts-20131226/xdebug.so" ]; then
...
if [ $TESTOTOOL != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		FILE1=$filename
		[ $? == 1 ] && exit 0;
		if [ "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" -a "$FILE1" != "/sbin/yubikey_shell" ]; then
